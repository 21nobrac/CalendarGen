...
class LunarCalendarState:
    def __init__(self, month_num=12, start_day=0):
...
def compute_lunar_calendars(row, lc_state_a, lc_state_b):
    if lc_state_a and lc_state_a.started:
        ...
        row["LunarA_Year"] = lc_state_a.year + 1
        row["LunarA_Month_#"] = month + 1
    ...
    if lc_state_b and lc_state_b.started:
        ...
        row["LunarB_Year"] = lc_state_b.year + 1
        row["LunarB_Month_#"] = month + 1
        row["LunarB_Month"] = month_name
        row["LunarB_Day"] = lc_state_b.day + 1